    ...
    */
    ...
    public function login(Request $request)
    {
        ...
        if (empty($client)) {
            ...
            return view('admin.auth.login')
                ->withErrors(['email'=>['Invalid login. Please enter a valid email address.']]);
        }
        if ($this->attemptLogin($request)) {
            //get first url
            $this->setDefaultPage($client);
    ...
    private function setDefaultPage() {
        ...
        $alowMenu = (new MenuDAO())->getAllBy($client);
        $default = '';

        foreach ($alowMenu as $menu) {
            if (empty($menu->parent_id)) {
                $default = $menu->route_name;
                break;
            }
        }
        if (!empty($default)) {
            $this->redirectTo = route($default);
        }
    }
