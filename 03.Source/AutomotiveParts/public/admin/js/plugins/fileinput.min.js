...
!function (e) {
    ...
    t = {
        ...
        constructor: i, _cleanup: function () {
            ...
            var s = this, d = s._getExifObj(l), c = null;
            return e.length && s.autoOrientImage && d && (c = d["0th"][piexif.ImageIFD.Orientation]), c ? (t.setImageOrientation(e, s.$preview.find("#zoom-" + a + " img"), c), s._raise("fileimageoriented", {
                $img: e,
                file: i
            ...
            r.loadedImages = [], r.totalImagesCount = 0, e.each(i, function (e, t) {
                ...
                var T, E, S, k, F, P, I, A, D, z, $, j, U = v + x, B = u + "-" + U, R = i[x], O = m.text, L = m.image,
                    M = m.html, Z = R && R.name ? r.slug(R.name) : "", N = (R && R.size || 0) / 1e3, H = "",
                    W = R ? t.objUrl.createObjectURL(R) : null, q = 0, V = "", K = 0, Y = function () {
                ...
                if (R) {
                    ...
                    if (!t.isEmpty(g) && t.isArray(g)) {
                        for (E = 0; E < g.length; E += 1) k = g[E], A = m[k], q += A && "function" == typeof A && A(R.type, R.name) ? 1 : 0;
                        if (0 === q) return S = r.msgInvalidFileType.setTokens({name: Z, types: V}), void y(S, R, B, x)
                    }
                    if (0 === q && !t.isEmpty(w) && t.isArray(w) && !t.isEmpty(H) && (F = t.compare(Z, H), q += t.isEmpty(F) ? 0 : F.length, 0 === q)) return S = r.msgInvalidFileExtension.setTokens({
                        name: Z,
                        extensions: _
                    }), void y(S, R, B, x);
                    ...
                    if (!C && N > b) return r.addToStack(R), s.addClass("file-thumb-loading"), r._previewDefault(R, B), r._initFileActions(), r._updateFileDetails(f), void a(x + 1);
                    o.length && void 0 !== FileReader ? (D = O(R.type, Z), z = M(R.type, Z), $ = L(R.type, Z), d.html(c.replace("{index}", x + 1).replace("{files}", f)), s.addClass("file-thumb-loading"), l.onerror = function (e) {
                        r._errorHandler(e, Z)
                    }, l.onload = function (i) {
                        ...
                        if (n = {name: Z, type: R.type}, e.each(m, function (e, t) {
